...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateEmeralds() {
        ...
        for (let i = emeralds.length - 1; i >= 0; i--) {
            ...
            // Use a larger radius for "pre-touch" collection
            const dx = player.position.x - e.position.x;
            const dz = player.position.z - e.position.z;
            const proximity = 5; // slightly bigger than collision box

            if (Math.abs(dx) < proximity && Math.abs(dz) < proximity) {
                // Optional: make it shrink/fade before collecting
                e.scaling.multiplyInPlace(new BABYLON.Vector3(0.8, 0.8, 0.8));
